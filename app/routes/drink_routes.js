...
router.get('/drinks/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Drink.findById(req.params.id)
    .then(handle404)
    // if `findById` is succesful, respond with 200 and "example" JSON
    .then(drink => res.status(200).json({ drink: drink.toObject() }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
...
router.patch('/drinks/:id', requireToken, removeBlanks, (req, res, next) => {
  // if the client attempts to change the `owner` property by including a new
  // owner, prevent that by deleting that key/value pair
  delete req.body.drinks.owner
  ...
  Drink.findById(req.params.id)
    ...
    .then(drink => {
      ...
      return drink.updateOne(req.body.drinks)
    })
